...
#TODO: Attention from UPPER and LOWER is not implemented yet, only from UPPER (needs to be done in both pyramidial and non pyramidial fashion)
class AttentionArchitecture(nn.Module):
    def __init__(self, in_channels_list, out_channels, mode):
        ...
        if mode == NeckMode.CROSSATTENTION_FROMUPPER_PYRAMIDIAL:
            #TODO: This is not correct probably
            self.keys_highestlevel = nn.Linear(in_channels_list[-1], d_model)
            self.values_highestlevel = nn.Linear(in_channels_list[-1], d_model)

        elif mode == NeckMode.CROSSATTENTION_FROMUPPERANDLOWER_HORIZONTAL:
        ...
        elif mode == NeckMode.CROSSATTENTION_FROMUPPER_HORIZONTAL:
            self.k_projs = nn.ModuleList([nn.Linear(in_ch, d_model) for in_ch in in_channels_list[1:]])
            self.v_projs = nn.ModuleList([nn.Linear(in_ch, d_model) for in_ch in in_channels_list[1:]])
    ...
    def forward(self, input):
        if self.mode == NeckMode.CROSSATTENTION_FROMUPPER_PYRAMIDIAL:
            ...
            return self.forward_upperandlower_horizontal(input)
        elif self.mode == NeckMode.CROSSATTENTION_FROMUPPER_HORIZONTAL:
            return self.forward_onlyupper_horizontal(input)
        else:
    ...
    #TODO: Weighting is not being done correct here I think, we also need to reweight intermediate feature maps (which are reused)
    def forward_onlyupper_pyramidial(self, input):
        ...
        for i in range(self.num_levels - 2, -1, -1):
            ...
            if pad_h or pad_w:
                ...
                #TODO: Is this OK? We need to make sure that Ht, Wt = 2 * Hk, 2 * Wk is given, else our approach does not work (Especially necessary at Inference / Evaluation time!)
        ...
        kv_flat = input[-1].flatten(2).transpose(1, 2)  # (B, Nc, Cin_kv)
        K_highestlevel = self.keys_highestlevel(kv_flat)
        V_highestlevel = self.values_highestlevel(kv_flat)
    ...
    ## Not pyramidial Version of forward(), but 'parallel' / horizontal implementation
    def forward_onlyupper_horizontal(self, input):
        # input = OrderedDict, 128,256,512,2048(,256)
        input = list(input.values())
        # input = List, 128,256,512,2048(,256)
    ...
    def forward_upperandlower_horizontal(self, input):
        # input = OrderedDict, 128,256,512,2048(,256)
        input = list(input.values())
        # input = List, 128,256,512,2048(,256)

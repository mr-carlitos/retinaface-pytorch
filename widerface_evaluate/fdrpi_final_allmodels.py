...
def evaluate_model(pred_dir: str, gt_path: str):
    ...
    for setting_id in range(3):
        ...
        pbar = tqdm.tqdm(range(len(event_list)), leave=False,
                         desc=f'[{os.path.basename(pred_dir)}] {settings[setting_id].capitalize():6}')
        ...
        pos_sorted = np.sort(positives)[::-1]
        neg_sorted = np.sort(negatives)[::-1]

        pos_counts = len(pos_sorted) - np.searchsorted(pos_sorted[::-1], thr, side='right')
        neg_counts = len(neg_sorted) - np.searchsorted(neg_sorted[::-1], thr, side='right')
...
def main(checkpoints_root: str, gt_path: str, out_dir: str):
    ...
    for model_path in model_dirs:
        ...
        try:
            curves = evaluate_model(os.path.join(model_path, 'like-mxnet'), gt_path)
    ...
    for idx, setting in enumerate(settings):
        ...
        plt.title(f'F-ROC – {setting.capitalize()} set')
        plt.ylim(0, 1)
        plt.legend(loc='lower left', fontsize=9)
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, f'froc_{setting}.png'), dpi=300)
        plt.close()
        print(f'✓ saved {setting} plot → {out_dir}/froc_{setting}.png')


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Compare all RetinaFace checkpoints.')
    parser.add_argument('--checkpoints', '-c',
                        default='/home/user/ckirchdorfer/carlos-workspace/Pytorch_Retinaface/save-checkpoints',
                        help='Folder that contains individual checkpoint dirs.')
    ...
    parser.add_argument('--out', '-o',
                        default='/home/user/ckirchdorfer/carlos-workspace/Pytorch_Retinaface/widerface_evaluate/plots',
                        help='Where to store the generated PNGs.')
    args = parser.parse_args()
    main(args.checkpoints, args.gt, args.out)
